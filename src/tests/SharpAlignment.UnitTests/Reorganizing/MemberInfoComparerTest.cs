...
public class MemberInfoComparerTest
{
    ...
    [InlineData("A", "a", -1)]
    [InlineData("aA", "Aa", 1)]
    public void OrderByIdentifierCaseSensitiveWhenConfigured(
        string left,
        string right,
        int expected
    )
    {
        ...
        var comparer = new MemberInfoComparer(
            new MemberSortConfiguration { SortByAlphabetCaseSensitive = true }
        );

        var result = comparer.Compare(leftMemberInfo, rightMemberInfo);
