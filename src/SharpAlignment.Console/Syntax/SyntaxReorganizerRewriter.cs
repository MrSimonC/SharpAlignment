...
public class SyntaxReorganizerRewriter : CSharpSyntaxRewriter
{
    ...
    public SyntaxReorganizerRewriter(
        IComparer<MemberInfo> memberInfoComparer,
        IComparer<UsingInfo> usingInfoComparer
    )
    {
    ...
    private SyntaxList<UsingDirectiveSyntax> OrganizeUsings(
        IEnumerable<UsingDirectiveSyntax> usingDirectives
    )
    {
        var sorted = usingDirectives.OrderBy(UsingInfoFactory.GetUsingInfo, _usingInfoComparer).ToSyntaxList();
        return sorted;
    }
}